...
// Chorros de polvo estelar
function createJetParticles(direction = 1) {
  const count = 3000;
  ...
  for (let i = 0; i < count; i++) {
    ...
    data.push({ spread, angle, dist, speed, t });
    positions[i * 3] = Math.cos(angle) * spread;
    positions[i * 3 + 1] = direction * dist;
    positions[i * 3 + 2] = Math.sin(angle) * spread;
    const bright = 0.2 + t * 0.8;
    const hue = 0.55 + Math.random() * 0.1;
    const rgb = new THREE.Color().setHSL(hue, 0.6, 0.4 + bright * 0.5);
    colors[i * 3] = rgb.r;
    colors[i * 3 + 1] = rgb.g;
    colors[i * 3 + 2] = rgb.b;
  ...
  mesh.position.y = direction * 0.5;
  return { mesh, data };
}

const jetNorth = createJetParticles(1);
beamGroup.add(jetNorth.mesh);

const jetSouth = createJetParticles(-1);
beamGroup.add(jetSouth.mesh);
...
async function initAudio() {
  try {
    ...
    const response = await fetch("audio/crab-pulsar-sound.ogg");
...
muteBtn.addEventListener("click", () => {
  ...
  if (audioMuted) {
    playAudio();
    audioMuted = false;
    muteBtn.textContent = "\u{1F50A} Sonido";
  } else {
    stopAudio();
    audioMuted = true;
    muteBtn.textContent = "\u{1F507} Sonido";
  }
});
...
function animate() {
  ...
  if (playing) {
    ...
    function updateJet(jet, dir) {
      ...
      for (let i = 0; i < jet.data.length; i++) {
        const p = jet.data[i];
